 ...
 */
function superblyTagField(tagField,tags,preset,allowNewTags,showTagsNumber){
...
$(document).ready(function(){
	...
	$('.superblyTagfield').css('display', 'none');

	$('.superblyTagfield').each(function (event){
		var allowNewTags = true;
		var showTagsNumber = 10;
		var preset = new Array();
		var tags = ['Afla Romeo','Aston Martin','Audi','Bentley','Bugatti','BMW','Citroen','Cadillac',
							'Chevrolet','Dodge','Daewoo','Ferrari','Ford','Fiat','Honda','Hyundai','Jaguar',
							'Jeep','Kia','Lada','Lexus','Lotus','Maserati','Mazda','Mercedes','Nissan','Opel','Peugeot',
							'Porsche','Rover','Renault','Suzuki','Subaru','Seat','Toyota','Volvo','Vauxhall','VW'];
							
							
		if($(this).attr('allowNewTags') == 'false'){
			allowNewTags = false;
		}

    	if($(this).attr('number') != null){
			showTagsNumber = $(this).attr('number');
		}
		...
		if($(this).attr('preset') != null){
			preset = $(this).attr('preset').split(',');
		}
		
		
		superblyTagField($(this),tags,preset,allowNewTags,showTagsNumber);
	});
